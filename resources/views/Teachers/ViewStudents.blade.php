        ...
        <form action="{{ route('teacher.view.students', ['subject' => $selectedSubject->subject_code]) }}" method="GET" class="mb-4">
            <div class="flex flex-wrap space-x-4">
                ...
                <div class="flex items-end">
                    ...
                    <a href="{{ route('teacher.view.students', ['subject' => $selectedSubject->subject_code, 'show_all' => true]) }}" class="ml-2 bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-md">Show All</a>
                </div>
            </div>
        </form>
                ...
                <table class="min-w-full leading-normal">
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-gray-100">
                            <td class="px-6 py-4 border-b border-gray-200">{{ $student->first_name }}</td>
                            <td class="px-6 py-4 border-b border-gray-200">{{ $student->last_name }}</td>
                            <td class="px-6 py-4 border-b border-gray-200">{{ $student->student_id }}</td>
                            <td class="px-6 py-4 border-b border-gray-200">{{ $student->email }}</td>
                            <td class="px-6 py-4 border-b border-gray-200">{{ $student->block }}</td>
                            <td class="px-6 py-4 border-b border-gray-200">{{ $student->year }}</td>
                            <td class="px-6 py-4 border-b border-gray-200">{{ $student->course }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
